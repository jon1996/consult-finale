...
if ($(".showcase-style-4").length > 0) {
  ...
  });  // Ensure mobile background coverage
  function ensureMobileBackgroundCoverage() {
    $(".showcase-style-4 .media").each(function() {
      ...
      // Additional mobile-specific styling for better coverage
      if ($(window).width() <= 768) {
        $media.css({
          'transform': 'scale(1.2)'
        });
    ...
    // Ensure showcase container covers hero section properly
    $(".showcase-style-4").css({
      'width': '100%',
      'overflow': 'hidden'
    });
  }
  ...
  $(window).on('load resize', ensureMobileBackgroundCoverage);
  ensureMobileBackgroundCoverage();
